...
### ENVIRONMENT ###
picard=/data/gv_h/lzhang/apps/picard.jar
refgenome=/data/gv_h/lzhang/references/homo_sapiens/GRCh38.p13/star-indices
gene=/data/gv_h/lzhang/references/homo_sapiens/GRCh38.p13/Homo_sapiens.GRCh38.104.gtf
genome=/data/gv_h/lzhang/references/homo_sapiens/GRCh38.p13/Homo_sapiens.GRCh38.dna.primary_assembly.fa
refname=/data/gv_h/lzhang/references/homo_sapiens/GRCh38.p13/Homo_sapiens.GRCh38.104.rsem
np=8
sn=${1}
readends=$(seq 1 2)

## the start ###
echo "RS pipeline version 2022.02.18 by Xiao Dong"; echo "Sample ID: "${sn}; echo "Processing starts on"; date; echo "==========";
...
fastqc -o ./alignment/1-1-fastqc ./fastq/${sn}-r${i}.fq.gz
done
...
fastq_screen \
  --aligner bowtie2 \
  --conf /data/gv_h/lzhang/apps/fastq_screen-0.14.1/fastq_screen_bowtie2.conf \
...
STAR \
  --runThreadN ${np} \
  --genomeDir ${refgenome} \
  --readFilesIn ./fastq/${sn}-r1.fq.gz ./fastq/${sn}-r2.fq.gz \
  --readFilesCommand zcat \
  --outFileNamePrefix ./alignment/1-3-human/${sn} \
  --outTmpDir ./alignment/1-3-human/${sn} \
  --outSAMtype BAM SortedByCoordinate \
  --quantMode TranscriptomeSAM
#  --soloType SmartSeq \ # not in use; "Segmentation fault (core dumped)" error.
#  --soloUMIdedup Exact
...
fastqc -o ./alignment/1-3-human/ ./alignment/1-3-human/${sn}.transcriptome.star.bam

## cleanup STAR genome alignment (not in use; for mutation calling, check transcriptome alignment above too) ##
java -Xmx20g -jar $picard AddOrReplaceReadGroups \
  VALIDATION_STRINGENCY=SILENT \
  TMP_DIR=`pwd`/tmp \
  I=./alignment/1-3-human/${sn}Aligned.sortedByCoord.out.bam \
  O=./alignment/1-3-human/${sn}.rd.bam \
  RGID=${sn} RGLB=${sn} RGPL=illumina RGPU=${sn} RGSM=${sn}

java -Xmx20g -jar $picard SortSam \
  VALIDATION_STRINGENCY=SILENT \
  TMP_DIR=`pwd`/tmp \
  I=./alignment/1-3-human/${sn}.rd.bam \
  O=./alignment/1-3-human/${sn}.sort.bam \
  SO=coordinate \
  CREATE_INDEX=true

java -Xmx20g -jar $picard MarkDuplicates \
  TMP_DIR=`pwd`/tmp \
  I=./alignment/1-3-human/${sn}.sort.bam \
  O=./alignment/1-3-human/${sn}.genome.star.bam \
  METRICS_FILE=./alignment/1-3-human/${sn}.genome.dedup_metrics.txt \
  CREATE_INDEX=true \
  REMOVE_DUPLICATES=true

rm ./alignment/1-3-human/${sn}Aligned.sortedByCoord.out.bam \
  ./alignment/1-3-human/${sn}.rd.* \
  ./alignment/1-3-human/${sn}.sort.* 

fastqc -o ./alignment/1-3-human/ ./alignment/1-3-human/${sn}.genome.star.bam
...
fractionreads_r1_hs=$(grep 'Human' alignment/1-2-fastq_screen/${sn}-r1_screen.txt | sed -n '1p' | awk '{print $6}')
fractionreads_r2_hs=$(grep 'Human' alignment/1-2-fastq_screen/${sn}-r2_screen.txt | sed -n '1p' | awk '{print $6}')

# col 5: % reads in r1 aligned to Mouse ref genome
# col 6: % reads in r2 aligned to Mouse ref genome
fractionreads_r1_mm=$(grep 'Mouse' alignment/1-2-fastq_screen/${sn}-r1_screen.txt | sed -n '1p' | awk '{print $6}')
fractionreads_r2_mm=$(grep 'Mouse' alignment/1-2-fastq_screen/${sn}-r2_screen.txt | sed -n '1p' | awk '{print $6}')

# col 7: % reads in r1 aligned to Yeast ref genome
# col 8: % reads in r2 aligned to Yeast ref genome
fractionreads_r1_ye=$(grep 'Yeast' alignment/1-2-fastq_screen/${sn}-r1_screen.txt | sed -n '1p' | awk '{print $6}')
fractionreads_r2_ye=$(grep 'Yeast' alignment/1-2-fastq_screen/${sn}-r2_screen.txt | sed -n '1p' | awk '{print $6}')

# col 9: % reads in r1 aligned to Ecoli ref genome
# col 10: % reads in r2 aligned to Ecoli ref genome
fractionreads_r1_ec=$(grep 'Ecoli' alignment/1-2-fastq_screen/${sn}-r1_screen.txt | sed -n '1p' | awk '{print $6}')
fractionreads_r2_ec=$(grep 'Ecoli' alignment/1-2-fastq_screen/${sn}-r2_screen.txt | sed -n '1p' | awk '{print $6}')

# col 11: % reads in r1 aligned to PhiX ref genome
# col 12: % reads in r2 aligned to PhiX ref genome
fractionreads_r1_px=$(grep 'PhiX' alignment/1-2-fastq_screen/${sn}-r1_screen.txt | sed -n '1p' | awk '{print $6}')
fractionreads_r2_px=$(grep 'PhiX' alignment/1-2-fastq_screen/${sn}-r2_screen.txt | sed -n '1p' | awk '{print $6}')

# col 13: % reads in r1 aligned to Lambda ref genome
# col 14: % reads in r2 aligned to Lambda ref genome
fractionreads_r1_lm=$(grep 'Lambda' alignment/1-2-fastq_screen/${sn}-r1_screen.txt | sed -n '1p' | awk '{print $6}')
fractionreads_r2_lm=$(grep 'Lambda' alignment/1-2-fastq_screen/${sn}-r2_screen.txt | sed -n '1p' | awk '{print $6}')
